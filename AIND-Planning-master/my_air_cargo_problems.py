...
class AirCargoProblem(Problem):
    ...
    def result(self, state: str, action: Action):
        ...
        for v_action in valid_actions:
            if v_action.precond_neg == action.precond_neg and v_action.precond_pos == action.precond_pos:
                find = True
                break
        if (not find): raise Exception("Action pre-conditions not satisfied")
        # remove negative literals
        for clause in action.effect_rem:
            cur_state.pos.remove(clause)
            cur_state.neg.append(clause)
        # add positive literals
        for clause in action.effect_add:
            cur_state.pos.append(clause)
            cur_state.neg.remove(clause)
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        for clause in self.goal:
            if clause not in cur_state.pos:
                count += 1
        return count
